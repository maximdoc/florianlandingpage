...
import { handleThemeTransition, refreshIconColors } from '@/utils/themeUtils';
...
export default function ThemeToggle() {
  ...
  useEffect(() => {
    ...
    // Add a listener for theme changes in localStorage
    const handleStorageChange = () => {
      const storedTheme = localStorage.getItem('theme');
      if (storedTheme && storedTheme !== theme) {
        // Synchronize UI with localStorage if they're out of sync
  ...
  const toggleTheme = () => {
    // Prevent repeated theme switching during animation
    if (isChanging) return;
    
    setIsChanging(true);
    const newTheme = theme === 'dark' ? 'light' : 'dark';
    
    // Use our utility for theme transition
    handleThemeTransition(() => {
      ...
      // Refresh icons
      setTimeout(() => {
        refreshIconColors('svg');
      }, 50);
    }, 700);
    
    // Enable button after transition completes
    setTimeout(() => {
      setIsChanging(false);
    }, 800);
  };
